...
const catLabel = slug => META.categories.find(c => c.slug === slug)?.label || slug;
const catOptions = sel => META.categories.map(c => `<option value="${c.slug}" ${c.slug === sel ? 'selected' : ''}>${esc(c.label)}</option>`).join('');
...
const views = {
  ...
  async dashboard() {
    ...
    view.innerHTML = `
      <div class="grid-4">
        ...
        <div class="stat-card"><div class="n">${s.enquiries.total}</div><div class="l">Total enquiries</div></div>
        <div class="stat-card"><div class="n">${s.gallery}</div><div class="l">Gallery photos</div></div>
        <div class="stat-card"><div class="n">${s.services}</div><div class="l">Services</div></div>
      </div>
      <div class="card">
        <div class="card-head"><h2>Recent <em>enquiries</em></h2><a class="btn btn-ghost btn-sm" href="#/enquiries">See all</a></div>
        ${s.recent.length ? enquiryTable(s.recent) : '<p class="empty">No enquiries yet. When someone submits the contact form, it shows up here.</p>'}
      </div>
      <div class="card">
        <div class="card-head"><h2>Quick <em>actions</em></h2></div>
        <div class="actions">
          <a class="btn btn-primary" href="#/gallery">Upload photos</a>
  ...
  async gallery() {
    ...
    view.innerHTML = `
      <div class="card">
        <div class="card-head"><h2>Upload <em>photos</em></h2><span class="dim">JPEG, PNG, WebP, HEIC · max 25 MB each · converted to WebP automatically</span></div>
        <div class="form-row-3">
          <div class="field"><label for="upCat">Category</label><select id="upCat">${catOptions('portraits')}</select></div>
          ${field('Title <small>optional — defaults to file name</small>', 'upTitle')}
          ${field('Alt text <small>optional — for accessibility & SEO</small>', 'upAlt')}
        </div>
      ...
      <div class="card">
        <div class="card-head"><h2>Photos <em>(${items.length})</em></h2><span class="dim">Drag to reorder · ★ marks the three featured photos on the homepage</span></div>
        ${items.length ? `<div class="gal-grid" id="galGrid">${items.map(galleryCard).join('')}</div>` : '<p class="empty">No photos yet.</p>'}
      </div>`;
    ...
    async function uploadFiles(files) {
      ...
      let done = 0;
      try {
        for (const f of files) {                       // one at a time so progress is meaningful
          const fd = new FormData();
          fd.append('photos', f); fd.append('category', $('#upCat').value);
          if (files.length === 1) { fd.append('title', $('#f_upTitle').value); fd.append('alt', $('#f_upAlt').value); }
          await api('/gallery', { method: 'POST', form: fd });
    ...
    view.addEventListener('click', async e => {
      ...
      const card = btn.closest('.gal-item'), id = card.dataset.id, item = items.find(x => x.id == id);
      if (btn.dataset.act === 'delete') {
        if (!confirmIt(`Delete "${item.title}"? This can't be undone.`)) return;
        await api(`/gallery/${id}`, { method: 'DELETE' }); toast('Photo deleted.'); views.gallery();
      }
      if (btn.dataset.act === 'feature') {
        await api(`/gallery/${id}`, { method: 'PATCH', body: { featured: !item.featured } });
        toast(item.featured ? 'Removed from featured.' : 'Added to featured.'); views.gallery();
      }
      if (btn.dataset.act === 'edit') {
        openModal(`
          <h2>Edit <em>photo</em></h2>
          <form id="galEdit" style="margin-top:18px">
            ${field('Title', 'title', item.title)}
            ${field('Alt text', 'alt', item.alt)}
            <div class="field"><label>Category</label><select name="category">${catOptions(item.category)}</select></div>
            <label class="check"><input type="checkbox" name="featured" ${item.featured ? 'checked' : ''}> Show in the featured trio on the homepage</label>
            <div class="actions" style="margin-top:20px"><button class="btn btn-primary" type="submit">Save changes</button></div>
          </form>`);
        $('#galEdit').addEventListener('submit', async ev => {
          ev.preventDefault();
          const fd = new FormData(ev.target);
          await api(`/gallery/${id}`, { method: 'PATCH', body: { title: fd.get('title'), alt: fd.get('alt'), category: fd.get('category'), featured: fd.get('featured') === 'on' } });
          closeModal(); toast('Saved.'); views.gallery();
        });
      }
    });

    /* drag reorder */
    enableDragSort('#galGrid', '.gal-item', async ids => { await api('/gallery/order', { method: 'PUT', body: { ids } }); toast('Order saved.'); });
  },
...
function galleryCard(g) {
  return `<div class="gal-item" data-id="${g.id}" draggable="true">
    <div class="img"><img src="${esc(g.src_thumb)}" alt="" loading="lazy">${g.featured ? '<span class="badge new fx">★ featured</span>' : ''}</div>
    <div class="meta"><b title="${esc(g.title)}">${esc(g.title)}</b><span>${esc(catLabel(g.category))}${g.external ? ' · placeholder' : ''}</span></div>
    <div class="row"><button class="icon-btn" data-act="edit" title="Edit">✎</button><button class="icon-btn" data-act="feature" title="Toggle featured">${g.featured ? '★' : '☆'}</button><button class="icon-btn danger" data-act="delete" title="Delete">✕</button></div>
  </div>`;
}
...
async function route() {
  ...
  try { await fn(); } catch (e) { view.innerHTML = `<div class="card"><p class="form-error">${esc(e.message)}</p></div>`; }
