...
export interface SonarGridProps extends React.ComponentProps<"div"> {
  ...
  /**
   * Where taps are listened for (read once on mount). "self" = the grid element (default).
   * "window" = anywhere on the page — for a fixed background layer that sits behind other content.
   */
  pointerTarget?: "self" | "window"
  /** Return false to ignore a tap, e.g. taps on text, links or panels. */
  shouldPing?: (event: PointerEvent) => boolean
}
...
export function SonarGrid({
  ...
  const opts = React.useRef({ spacing, dotRadius, baseOpacity, pingEvery, speed, ringWidth, amplitude, interactive, maxRings, seedPing, pingArea, shouldPing })
  opts.current = { spacing, dotRadius, baseOpacity, pingEvery, speed, ringWidth, amplitude, interactive, maxRings, seedPing, pingArea, shouldPing }
  ...
  React.useEffect(() => {
    ...
    const pointerHost: EventTarget = pointerTarget === "window" ? window : host
    ...
    const onDown = (e: Event) => {
      ...
      if (opts.current.shouldPing && !opts.current.shouldPing(event)) return
      const rect = host.getBoundingClientRect()
    ...
    pointerHost.addEventListener("pointerdown", onDown)
    ...
    return () => {
      ...
      io.disconnect()
      mo.disconnect()
      pointerHost.removeEventListener("pointerdown", onDown)
    ...
    // pointerTarget is read once on mount (documented on the prop).
  ...
  return (
    <div
      ...
      className={cn("relative isolate overflow-hidden", interactive && pointerTarget === "self" && "cursor-crosshair", className)}
      {...rest}
    >
      <canvas
        ref={canvasRef}
