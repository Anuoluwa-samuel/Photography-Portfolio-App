...
async function sendEnquiryNotification(e, siteName) {
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;background:#0b0d0e;color:#f2f4f4;padding:32px">
      <p style="color:#f5b82e;letter-spacing:.2em;font-size:12px;text-transform:uppercase;margin:0 0 8px">${esc(siteName)}</p>
      ...
      <p style="margin-top:32px"><a href="mailto:${esc(e.email)}" style="background:#f5b82e;color:#0b0d0e;padding:12px 20px;border-radius:6px;text-decoration:none;font-weight:600">Reply to ${esc(e.name.split(' ')[0])}</a></p>
    </div>`;
