...
module.exports = {
  ...
  get(id)      { return db.prepare(`${BASE_SELECT} WHERE p.id = ?`).get(id); },
  bySlug(slug) { return db.prepare(`${BASE_SELECT} WHERE p.slug = ?`).get(slug); },
  slugExists(slug) { return !!db.prepare('SELECT 1 FROM projects WHERE slug = ?').get(slug); },
  count() { return db.prepare('SELECT COUNT(*) AS n FROM projects').get().n; },

  create(p) {
    const sort = (db.prepare('SELECT COALESCE(MAX(sort), -1) AS m FROM projects').get().m) + 1;
    return db.prepare(`INSERT INTO projects (title, slug, description, category_id, location, date, cover_image, featured, published, sort)
                       VALUES (@title, @slug, @description, @category_id, @location, @date, @cover_image, @featured, @published, @sort)`)
             .run({ description: '', category_id: null, location: '', date: '', cover_image: '', featured: 0, published: 1, ...p, sort }).lastInsertRowid;
  },
  update(id, p) {
