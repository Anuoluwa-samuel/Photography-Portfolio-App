...
function list(req, res) {
  res.json({ items: Project.all(), categories: Category.all() });
}
...
function get(req, res) {
  ...
  res.json({ item: p, images: Image.forProject(p.id) });
}
...
function create(req, res) {
  ...
  const slug = uniqueSlug(clean(b.slug, 80) || title, s => Project.slugExists(s));
  const data = { ...projectFromBody(b), title, slug };
  const id = Project.create(data);
  res.status(201).json({ ok: true, item: Project.get(id) });
}

function update(req, res) {
  const cur = Project.get(req.params.id);
  ...
  if (b.slug !== undefined) {
    ...
    slug = requested === cur.slug ? cur.slug : uniqueSlug(requested, s => Project.slugExists(s));
  }
  const data = { ...projectFromBody(b, cur), title, slug, id: cur.id };
  Project.update(cur.id, data);
  res.json({ ok: true, item: Project.get(cur.id) });
}

function remove(req, res) {
  const p = Project.get(req.params.id);
  if (!p) return res.status(404).json({ success: false, message: 'Project not found', error: 'PROJECT_NOT_FOUND' });
  for (const img of Image.forProject(p.id)) {
    imageService.removeGalleryFile(img.src_full);
    imageService.removeGalleryFile(img.src_thumb);
  }
  Project.remove(p.id); // ON DELETE CASCADE removes the image rows
  res.json({ ok: true });
}
...
async function uploadImages(req, res) {
  ...
  for (const file of req.files) {
    ...
    const id = Image.create({ project_id: project.id, ...processed, caption: clean(req.body.caption, 120), alt: clean(req.body.alt, 300) });
    created.push(Image.get(id));
  }
  if (!project.cover_image) Project.setCover(project.id, created[0].src_thumb);
  res.status(201).json({ ok: true, items: created, project: Project.get(project.id) });
}

function updateImage(req, res) {
  const img = Image.get(req.params.imageId);
  ...
  const b = req.body || {};
  Image.update(img.id, { caption: clean(b.caption ?? img.caption, 120), alt: clean(b.alt ?? img.alt, 300) });
  if (b.cover) Project.setCover(img.project_id, img.src_thumb);
  res.json({ ok: true, item: Image.get(img.id), project: Project.get(img.project_id) });
}
...
function removeImage(req, res) {
  ...
  imageService.removeGalleryFile(img.src_thumb);
  Image.remove(img.id);
  const project = Project.get(img.project_id);
  if (project.cover_image === img.src_thumb) {
    const next = Image.forProject(project.id)[0];
    Project.setCover(project.id, next ? next.src_thumb : '');
  }
  res.json({ ok: true, project: Project.get(img.project_id) });
}

function reorderImages(req, res) {
  const ids = (req.body?.ids || []).map(Number).filter(Number.isInteger);
  Image.reorder(Number(req.params.id), ids);
  res.json({ ok: true });
}
